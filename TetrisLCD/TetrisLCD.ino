...
void loop() {
  ...
  if(gameState.lockedAboveBoard()) gameOver(); //if a piece locks above theboard, reset the game
...
void(* resetFunc) (void) = 0; //creates a function that resets the arduino

void gameOver(void){
  display.gameOver(); //displays the game over screen
  resetFunc(); //resets the arduino to the begining of the game
}
